...
public class ElementLR extends Element {
	private int rotacija;
	
	public ElementLR(int pocetak, int velicina) {
		super(new Point(pocetak, -90), velicina);
		
		for (int i = 0; i < 4; i++) {
			Point tacka = rotPozicija(0, i);
			delovi.add(new Kvadrat(new Color(68,22,63), tacka.x, tacka.y, velicina, new Color(53,14,46)));
		}
	}
